...
echo "- Burning bootloader via ISP..."
 /home/pi/arduino-1.8.19/hardware/tools/avr/bin/avrdude -C /home/pi/arduino-1.8.19/hardware/tools/avr/etc/avrdude.conf -uq -p atmega328p -c usbasp -P usb -U flash:w:/home/pi/factory-test/bootloaders/optiboot_atmega328.hex:i -Ulock:w:0x0F:m -l /home/pi/factory-test/avrdude.log
...
check="/dev/serial/by-id/"$(ls /dev/serial/by-id)
if [ "$check" = "$uartprogrammer" ]; then
    echo "- Uploading firmware via UART..."
